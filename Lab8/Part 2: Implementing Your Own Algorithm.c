...
int checkformove(char board[][26], int n, int row, int col,
                           char hc, char cc){
...
void computerMoves1(char board[26][26], int n, char cc, char hc, int *row, int *col)
{
    int dr, dc;
    int count = 0;
    int bestRow = 0;
  ...
  for(int z = 0; z < n; z++){
        ...
        for (int col1 = 0; col1 < n; col1++)
        {
            if (board[row1][col1] == 'U')
            {
                for (dr = -1; dr <= 1; dr++)
                {
                    for (dc = -1; dc <= 1; dc++)
                    {
                        if((dr != 0 || dc != 0) && checkLegalInDirection(board, n, row1, col1, cc, dr, dc))
                        {
    ...
    for (int row1 = 0; row1 < n; row1++)
    {
        for (int col1 = 0; col1 < n; col1++)
        {
            if(validMovesComputer[row1][col1] == 0)
             continue;
         for(i = 0; i < n; i++)
          for(j = 0; j < n; j++)
           tempboard[i][j] = board[i][j];
        ...
        for (int col1 = 0; col1 < n; col1++)
        {
            if (board[row1][col1] == 'U')
            {
                for (dr = -1; dr <= 1; dr++)
                {
                    for (dc = -1; dc <= 1; dc++)
                    {
                        if((dr != 0 || dc != 0) && checkLegalInDirection(board, n, row1, col1, hc, dr, dc))
                        {
...
void computerMoves(char board[26][26], int n, char cc, char hc)
{
  ...
  for(int z = 0; z < n; z++){
        ...
        for (int col = 0; col < n; col++)
        {
            if (board[row][col] == 'U')
            {
                for (dr = -1; dr <= 1; dr++)
                {
                    for (dc = -1; dc <= 1; dc++)
                    {
                        if((dr != 0 || dc != 0) && checkLegalInDirection(board, n, row, col, cc, dr, dc))
                        {
    ...
    for (int row = 0; row < n; row++)
    {
        for (int col = 0; col < n; col++)
        {
            if(validMovesComputer[row][col] == 0)
             continue;
         for(i = 0; i < n; i++)
          for(j = 0; j < n; j++)
           tempboard[i][j] = board[i][j];
        ...
        for (int col = 0; col < n; col++)
        {
            if (board[row][col] == 'U')
            {
                for (dr = -1; dr <= 1; dr++)
                {
                    for (dc = -1; dc <= 1; dc++)
                    {
                        if((dr != 0 || dc != 0) && checkLegalInDirection(board, n, row, col, hc, dr, dc))
                        {
